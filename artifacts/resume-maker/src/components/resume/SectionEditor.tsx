...
function findSummary(allSections?: SectionEditorProps["allSections"]) {
  return (allSections?.find((s) => s.type === "summary")?.content?.text as string) ?? "";
}
function jobTitleFrom(personal: SectionContent): string {
  return ((personal.jobTitle as string) || (personal.title as string) || "").trim();
}
...
function readSocials(content: SectionContent): SocialItem[] {
  ...
  if ((content.github as string)?.trim()) out.push({ label: "GitHub", url: content.github as string });
  if ((content.linkedin as string)?.trim()) out.push({ label: "LinkedIn", url: content.linkedin as string });
  if ((content.twitter as string)?.trim()) out.push({ label: "Twitter", url: content.twitter as string });
  return out;
}

function PersonalEditor({ content, onChange }: { content: SectionContent; onChange: (c: SectionContent) => void }) {
  ...
  return (
    <div className="space-y-3">
        ...
        <div className="flex items-center gap-3">
          <div className="h-14 w-14 rounded-full overflow-hidden bg-muted flex items-center justify-center text-base font-bold text-muted-foreground border border-border shrink-0">
            {photo
              ...
              : ((content.name as string) ?? "").trim().split(/\s+/).map((p) => p[0]).slice(0, 2).join("").toUpperCase() || "?"}
          </div>
          <div className="flex flex-col gap-1.5">
            ...
            />
            <Button type="button" variant="outline" size="sm" className="h-7 text-xs gap-1.5" onClick={() => fileRef.current?.click()}>
              <Upload className="h-3 w-3" />
              {photo ? "Replace photo" : "Upload photo"}
            </Button>
            {photo && (
              <Button type="button" variant="ghost" size="sm" className="h-6 text-[10px] gap-1 text-muted-foreground hover:text-destructive justify-start" onClick={() => update("photo", "")}>
      ...
      <Field label="Full Name">
        <Input value={(content.name as string) ?? ""} onChange={(e) => update("name", e.target.value)} placeholder="Jane Smith" />
      </Field>
      ...
      <div className="grid grid-cols-2 gap-2 items-end">
        ...
        <Field label="Phone">
          <Input value={(content.phone as string) ?? ""} onChange={(e) => update("phone", e.target.value)} placeholder="+1 555 000 0000" />
        </Field>
      </div>
      <Field label="Location">
        <Input value={(content.location as string) ?? ""} onChange={(e) => update("location", e.target.value)} placeholder="San Francisco, CA" />
      </Field>
...
function SocialsEditor({ content, onChange }: { content: SectionContent; onChange: (c: SectionContent) => void }) {
  ...
  const writeSocials = (next: SocialItem[]) => {
    ...
    const { github: _g, linkedin: _l, twitter: _t, ...rest } = content as Record<string, unknown>;
  ...
  return (
    <div className="pt-1 border-t border-border">
      <p className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground mt-3 mb-1">Social Links</p>
      <p className="text-[10.5px] text-muted-foreground mb-2">
        Add a label (e.g. <span className="font-medium">GitHub</span>, <span className="font-medium">Portfolio</span>) and a link.
        Only the label is shown on your resume — it becomes a clickable highlighted link.
      </p>
      <div className="space-y-1.5">
        {socials.map((s, i) => (
          <div key={i} className="flex gap-1.5 items-center">
            ...
            <button
              type="button"
              onClick={() => writeSocials(socials.filter((_, idx) => idx !== i))}
  ...
  const handleGenerate = () => {
    ...
    if (!jobTitle && !willRefine) {
      toast({
        ...
        description: "Set your job title in the Personal section, or type a draft below — the AI uses these as a starting point.",
        variant: "destructive",
      });
      return;
    }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-col gap-1">
        ...
        >
          {generateSummary.isPending ? <Loader2 className="h-3 w-3 animate-spin" /> : <Sparkles className="h-3 w-3 text-primary" />}
...
function ExperienceEditor({
  ...
  const addItem = () => {
    onChange({ ...content, items: [...items, { company: "", title: "", startDate: "", endDate: "", location: "", bullets: [""] }] });
  };
    ...
    <div className="space-y-4">
      {items.map((item, i) => {
        ...
        const removeBullet = (bi: number) =>
          updateItem(i, "bullets", rawBullets.filter((_, idx) => idx !== bi));
        return (
        <div key={i} className="rounded-lg border border-border p-3 space-y-2.5 relative">
          ...
          <div className="space-y-1.5">
            ...
            {rawBullets.map((raw, bi) => {
              ...
              const isImproving = improveBullet.isPending && pendingBullet?.itemIndex === i && pendingBullet?.bulletIndex === bi;
              return (
            ...
            <Button
              variant="outline" size="sm"
              className="gap-1 h-7 text-xs"
              onClick={() => updateItem(i, "bullets", [...rawBullets, { text: "", label: "", link: "" }])}
            >
...
function EducationEditor({ content, onChange }: { content: SectionContent; onChange: (c: SectionContent) => void }) {
  const items: Array<Record<string, unknown>> = (content.items as Array<Record<string, unknown>>) ?? [];
...
function SkillsEditor({
  ...
  const suggestSkills = useSuggestSkills({
    ...
    mutation: {
      onSuccess: (data) => {
        ...
        const fresh = (data.skills ?? []).filter((s) => s && !existing.has(s.toLowerCase()));
        if (fresh.length === 0) {
          toast({ title: "No new suggestions — try refining your job title or summary" });
          return;
        }
        setSuggested(fresh);
  ...
  const addPicked = () => {
    const newItems = suggested.filter((s) => picked.has(s)).map((s) => ({ name: s, level: 70 }));
    if (newItems.length > 0) {
      onChange({ ...content, items: [...items, ...newItems] });
      toast({ title: `Added ${newItems.length} skill${newItems.length === 1 ? "" : "s"}` });
    }
    setSuggested([]); setPicked(new Set());
  };
  const dismissSuggestions = () => { setSuggested([]); setPicked(new Set()); };

  const addSkill = () => {
    if (!newSkill.trim()) return;
    onChange({ ...content, items: [...items, { name: newSkill.trim(), level: 70 }] });
    setNewSkill("");
  };
  ...
  const handleSuggest = () => {
    ...
    if (!jobTitle) {
      toast({
        title: "Add a job title first",
        description: "Set your job title in the Personal section so the AI knows what skills to suggest.",
        variant: "destructive",
      });
      return;
    }
  ...
  return (
    <div className="space-y-3">
      ...
      >
        {suggestSkills.isPending ? <Loader2 className="h-3 w-3 animate-spin" /> : <Sparkles className="h-3 w-3 text-primary" />}
      ...
      {suggested.length > 0 && (
        <div className="rounded-md border border-primary/30 bg-primary/5 p-2.5 space-y-2">
          <div className="flex items-center justify-between">
            <p className="text-[11px] font-semibold text-foreground">Pick the skills you want to add</p>
            <span className="text-[10px] text-muted-foreground">{picked.size}/{suggested.length} selected</span>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {suggested.map((s) => {
          ...
          <div className="flex gap-2">
            <Button size="sm" className="h-7 text-xs flex-1" onClick={addPicked} disabled={picked.size === 0}>
              Add {picked.size > 0 ? `${picked.size} ` : ""}selected
            </Button>
            <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={dismissSuggestions}>
              Cancel
            </Button>
          </div>
        </div>
...
function CertificationsEditor({ content, onChange }: { content: SectionContent; onChange: (c: SectionContent) => void }) {
  const items: Array<Record<string, unknown>> = (content.items as Array<Record<string, unknown>>) ?? [];
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        ...
        <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-muted-foreground" onClick={onVisibilityToggle}>
          {section.isVisible !== false ? <Eye className="h-3.5 w-3.5" /> : <EyeOff className="h-3.5 w-3.5" />}
        </Button>
      </div>
      {section.isVisible === false ? (
        <p className="text-xs text-muted-foreground italic">This section is hidden from your resume.</p>
      ) : (
        <>
          {section.type === "personal" && <PersonalEditor content={section.content} onChange={onChange} />}
